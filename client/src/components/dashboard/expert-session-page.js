...
class ExpertSessionPage extends Component {
  static propTypes = {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      time: {},
      //seconds: 1800,
      seconds: this.props.cookies.get("selectMins")
        ? this.props.cookies.get("selectMins")
        : 1800,
    ...
    window.$(document).ready(function () {
      window.$(".Chat_Trigger").click(function () {
        window.$("body").toggleClass("OpenChatList");
      });
      ...
      window.$(document).on(
        "click",
        ".WhiteBoard_Trigger a,.whiteBoard_close",
        function () {
          window.$("body").toggleClass("OpenWhiteBoard");
          window.$(".whiteBoard_close").removeClass("OpenWhiteBoard");
        }
      );
      window.$(document).on("click", ".stream-end-btn", function () {
        window.$(this).parent().remove();
      });
      window.$(".canvas-tools a").click(function () {
        window.$(".canvas-tools a").removeClass("active");
    ...
    socket.on("expert user disconnected", (data) => {
      console.log(
        "client side : expert user disconnected " +
          this.state.time +
          " : " +
          this.state.seconds
      );
    ...
    this.state.currentUser = this.props.cookies.get("user");
  ...
  componentDidMount() {
    ...
    setTimeout(function () {
      var g = window.$(".emojionearea-editor")[0];
    ...
    var expertSlug = this.props.match.params.slug, //"avadhesh_bhatt@rvtechnologies.co.in",
    ...
    axios
      .post(`${API_URL}/createVideoSession`, {
        expertSlug,
        userEmail,
        sessionOwner,
      })
      .then((res) => {
                ...
                .then(
                  ...
                  (err) =>
                    err.response.json().then(({ errors }) => {
                      console.log(
                        "saveVideoSessionInfo err: " + JSON.stringify(err)
                      );
                    })
                );
            } catch (e) {
        ...
        session
          .on("streamCreated", function (event) {
            ...
            window.$("#" + subContainer.id + " .OT_root").append(
              '<button id="end-btn-' +
                event.stream.streamId +
                '" class="stream stream-end-btn">End Button</button>'
            );
            ...
            var subscriber = session.subscribe(
              ...
              function (error) {
                ...
                if (subscriber.stream.hasVideo) {
                  ...
                  // if(!imgData){
                  //     subscriber.setStyle('backgroundImageURI','https://donnieslist.com/img/mini-user-dummy.png');
                  // }
            ...
            if (
              ...
              if (self.state.currentUser) {
                ...
                if (self.state.currentUser.role === "User") {
                  ...
                  var userName =
                    self.state.currentUser.firstName +
                    " " +
                    self.state.currentUser.lastName;
                  var expertName =
                    self.state.expert.profile.firstName +
                    " " +
                    self.state.expert.profile.lastName;
                  var userImage = self.state.currentUser.profileImage
                    ? Image_URL + self.state.currentUser.profileImage
                    : CLIENT_ROOT_URL + "/img/profile.png";
                  var expertImage = self.state.expert.profileImage
                    ? Image_URL + self.state.expert.profileImage
                    : CLIENT_ROOT_URL + "/img/profile.png";
                  axios
                    .post(`${API_URL}/videosession/payment-video-session`, {
                      customer_id,
                      userEmail,
                      amount,
                      stripe_connect_id,
                      video_session_id,
                      videoSessionDuration,
                      expertEmail,
                      userName,
                      expertName,
                      userImage,
                    ...
                    .then((res) => {
                      ...
                      window.$(".wrapper").addClass("blur_page");
                    })
                    .catch((err) => {
                      console.log(
                        "*** payment video session : error " +
                          JSON.stringify(err)
                      );
                    });
          ...
          .on("connectionCreated", function (event) {
            ...
            if (
              ...
              console.log(
                "Another client connected. " + connectionCount + " total."
              );
              ...
              if (self.state.currentUser) {
                if (self.state.currentUser.role === "User") {
                  ...
                  self.setState({
                    ...
                    blurChatAreaStatusMessage:
                      "Waiting for Expert to connnect!",
                    infoAlertText:
                      "Expert is online, chat feature is available now",
                  });
                } else {
                  self.setState({
                    blurChatAreaStatus: false,
          ...
          .on(
            ...
            function connectionDestroyedHandler(event) {
              ...
              self.setState({
                isShowingInfoAlert: true,
                infoAlertText:
                  "Alas, " + connectionDestroyer + " left the session page.",
                blurChatAreaStatus: true,
                blurChatAreaStatusMessage:
                  "Waiting for " + connectionDestroyer + " to connnect!",
              });
            }
          )
          ...
          .on("signal:user_session_connected", function (event) {
            ...
            if (event.from.id !== session.connection.id) {
              ...
              self.setState({
                ...
                infoAlertText:
                  connected_username +
                  " is online, you can start session by clicking on call button.",
              ...
              self.props
                .FetchExpertConversation(
                  self.props.params.slug,
                  connected_useremail
                )
                .then((res) => {
                  window.$(".Chating_msg_Here").animate({
                    //                    scrollTo: window.$('.Chating_msg_Here ul').scrollHeight
                    scrollTop: window.$(".Chating_msg_Here ul").prop("scrollHeight"),
                  });
                });
            ...
            if (self.state.currentUser) {
              if (self.state.currentUser.role === "User") {
                var data = {
                  ...
                  connected_userimage: self.state.currentUser.profileImage
                    ? Image_URL + self.state.currentUser.profileImage
                    : CLIENT_ROOT_URL + "/img/profile.png",
                };

                if (this.props.cookies.get("selectMins")) {
                  this.props.cookies.remove("selectMins", { path: "/" });
                }
          ...
          .connect(this.state.apiToken, function (error) {
            if (error) {
              ...
              if (error.code === 1010) {
                window.$("#publisher").html(
                  '<div class="alert alert-danger">Alas! You are not connected to the internet. Check your network connection. <a data-toggle="modal" data-target="#myModalExtendSession" class="btn btn-primary"><i className="fa fa-handshake-o"></i> Extend Session Now!</a></div>'
                );
              } else if (error.code === 1004) {
                window.$("#publisher").html(
                  '<div class="alert alert-danger">Alas! Session Expired, Please recharge your account to restart session! <a data-toggle="modal" data-target="#myModalExtendSession" class="btn btn-primary"><i className="fa fa-handshake-o"></i> Extend Session Now!</a></div>'
                );
              }
            } else {
              ...
              var optionsPublisher = {
                name:
                  _this.state.currentUser.firstName +
                  " " +
                  _this.state.currentUser.lastName,
                "z-index": 2,
              ...
              setTimeout(function () {
                window.$("#" + publisher.id + " .OT_name").text("Me");
              }, 500);
              ...
              window.$(".session-page-container").show();
              window.$(".loader-center-ajax").hide();
              //window.$('.Left_Panel').css('height',window.$('.mainContainer').height()+'px');
              window.$(".Chating_msg_Here").animate({
                scrollTop: window.$(".Chating_msg_Here ul").prop("scrollHeight"),
              });
            }
      ...
      .catch((err) => {
        if (err) {
          ...
          this.setState({
            errorSessionMessage:
              "<div class='alert alert-danger'>Session is not active from Expert end. Please try again later.</div>",
          });
        }
      });
    ...
    console.log(
      "Stripe connect id " + this.state.currentUser.stripe_connect_id
    );
  }
          ...
          {this.props.messages.map((data, index) => (
            <li key={`DATA_${index}`}
              className={this.getClassName(
                data.messageSenderEmail,
                currentUser.email
              )}
            >
              <span className="message-body">{emojify(data.message)}</span>
              <div className="session-msg-date">
                {moment(data.messageTime).fromNow()}
              </div>
            </li>
          ))}
        </ul>
  ...
  startTimer() {
    ...
    if (!this.state.refreshIntervalId) {
      this.setState({ refreshIntervalId: setInterval(this.countDown, 1000) });
    }
  }
  stopTimer() {
    ...
    if (this.state.currentUser) {
      if (this.state.currentUser.role === "User") {
        ...
        this.state.sessionObj.signal(
          { type: "video_session_end", data: "video call session end by user" },
          function (err) {
            if (err) {
              console.log(
                "*** video session end by user : ERROR ***" +
                  JSON.stringify(err)
              );
            } else {
        ...
        // expert ended the session
        this.state.sessionObj.signal(
          {
            type: "video_session_end",
            data: "video call session end by expert",
          },
          function (err) {
            if (err) {
              console.log(
                "*** video session end by expert: ERROR ***" +
                  JSON.stringify(err)
              );
            } else {
  ...
  startVideoSession(e) {
    ...
    this.state.sessionObj.signal(
      { type: "video_session_start", data: "session started by expert" },
      function (err) {
        if (err) {
  ...
  onSubmitReview() {
    ...
    setTimeout(function () {
      ...
      this.props.history.push("/");
      window.$(".wrapper").removeClass("blur_page");
    }, 2000);
  }

  showSessionCallBtn() {
    if (this.state.currentUser.role === "Expert") {
      return this.state.sessionStartEndBtn === "start" ? (
        <li className="video_session_call_btn start_video_session_call">
          <Link
            to="#"
            data-toggle="tooltip"
            title="Start Session"
            onClick={(e) => this.startVideoSession(e)}
          ></Link>
        </li>
      ) : (
        <li className="video_session_call_btn end_video_session_call">
          <Link
            data-toggle="tooltip"
            title="End Session"
            to={void(0)}
            onClick={() => this.disconnectOT()}
          ></Link>
        </li>
      );
    }
    if (this.state.currentUser.role === "User") {
      return this.state.isExpertStartedSession ? (
        <li className="video_session_call_btn end_video_session_call">
          <Link
            data-toggle="tooltip"
            title="End Session"
        ...
        <li>
          <Link to="#" style={{ visibility: "hidden" }}></Link>
        </li>
      );
    }
  }
  ...
  render() {
    ...
    if (videoDisableWarning && subscriber) {
      window.$("#" + subscriber.id + " video").addClass("blur");
      if (window.$(".video-disconnect-error-container").length === 0) {
        window.$("#" + subscriber.id).append(
          "<div class='video-disconnect-error-container'><div>Connection unstable</div></div>"
        );
      }
    } else if (subscriber) {
      window.$("#" + subscriber.id + " video").removeClass("blur");
      window.$(".video-disconnect-error-container").remove();
    }
    ...
    return (
      <div className="session-page">
        ...
        <UserReview
          ...
          reviewModalData={this.state.reviewModalData}
          expertSlug={this.props.match.params.slug}
          showUserReviewModal={this.state.showUserReviewModal}
          reviewBy={this.state.currentUser.role}
        />
        <ErrorExpertNotActive
          errorExpertNotActiveModalData={
            this.state.errorExpertNotActiveModalData
          }
          expertSlug={this.props.match.params.slug}
          showErrorExpertNotActiveModal={
            this.state.showErrorExpertNotActiveModal
          }
        />
        <div
          className="container-fluid session-page-container"
          style={{ display: "none" }}
        >
          <div className="row">
            <section>
              <div
                dangerouslySetInnerHTML={{
                  __html: this.state.errorSessionMessage,
                }}
              />
              <div className="mainContainer FullHeightSec">
                <div className="Left_Panel">
                  <div
                    className={
                      this.state.blurChatAreaStatus
                        ? "Conversation cntrl_show"
                        : "Conversation cntrl_hide"
                    }
                  >
                    <h6>Conversation</h6>
                    <a href="#!" className="Hide_Left_panel">
                  ...
                  <p
                    className={
                      this.state.blurChatAreaStatus
                        ? "text-center chatarea_msg_show"
                        : "text-center chatarea_msg_hide"
                    }
                  >
                    {this.state.blurChatAreaStatusMessage}
                  </p>
                  <div
                    className={
                      this.state.blurChatAreaStatus
                        ? "Chat_Main_section chatarea_blur"
                        : "Chat_Main_section"
                    }
                  >
                    <div className="Chating_msg_Here">{this.renderInbox()}</div>
                    <div className="typeMessage">
                      {this.state.currentUser.role === "Expert" && (
                        <ExpertReplyMessage
                          sessionOwnerUsername={this.props.match.params.slug}
                          messageReceiverEmail={this.state.connected_useremail}
                          messageSenderEmail={currentUser.email}
                          expertEmail={this.state.expert.email}
                        />
                      )}
                      {this.state.currentUser.role === "User" && (
                        <ExpertReplyMessage
                          sessionOwnerUsername={this.props.match.params.slug}
                          messageReceiverEmail={this.state.expert.email}
                          messageSenderEmail={currentUser.email}
                          expertEmail={this.state.expert.email}
                        />
                      )}
                ...
                <div className="Right_Panel">
                  ...
                  <div className="footer_action_btns">
                    <a
                      href="#!"
                      data-toggle="modal"
                      data-target="#myModalWaitingToJoin"
                      className="btn btn-primary btn_waitingtojoin"
                      rel="noreferrer"
                    > </a>
                    <ul>
                      <li
                        className={
                          this.state.blurChatAreaStatus
                            ? "Chat_Trigger cntrl_hide"
                            : "Chat_Trigger cntrl_show"
                        }
                      >
                        <a
                          data-toggle="tooltip"
                      ...
                      <li
                        className={
                          this.state.blurChatAreaStatus
                            ? "WhiteBoard_Trigger cntrl_hide"
                            : "WhiteBoard_Trigger cntrl_show"
                        }
                      >
                      ...
                      {/* this.state.showEndCallOptions ? <li className="phone_call"><Link data-toggle="tooltip" title="End Session" to={void(0)} onClick={this.disconnect.bind(this)}><i className="fa fa-phone"></i></Link></li> : null */}
                      <li
                        className="clockicon"
                      ...
                      >
                        ...
                        >
                          ...
                          {this.state.sessionDate} - {this.state.time.h} :
          ...
          <div className="modal-dialog">
            <div className="modal-content alert alert-danger">
              ...
              <div className="modal-body text-center">
                <p>
                  No one has joined your session yet, once any user will join
                  the session, timer will initiate!
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p className="text-center">
                  ...
                  Recharge your account to start session!
                </p>
                      ...
                      <td>
                        <form>
                          ...
                          <div className="form-group">
                            <i>
                              Note: Transaction will process automatically in
                              backend!
                            </i>
                          </div>
                          <div className="form-group">
        ...
        {/* myModalExtendSession for email end here */}
      </div>
    );
  }
}
...
ExpertSessionPage.defaultProps = { redAlert: 5 };
...
export default connect(mapStateToProps, {
  ...
  ExpertSessionUserDisconnected,
  saveVideoSessionInfo,
})(withRouter(withCookies(ExpertSessionPage)));
