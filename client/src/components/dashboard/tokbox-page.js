...
class TokboxPage extends Component {
  ...
  _initOT(token, sessionId, apiKey) {
    var session = OT.initSession(apiKey, sessionId).on({
      sessionConnected: function (event) {
        ...
        var subscriber = session.subscribe(event.stream, "myDiv");
    ...
    //var join = this.props.match.params.join;
  ...
  publish(apiKey, session, token) {
    ...
    var publisher = OT.initPublisher(apiKey, "publisherA", {
      width: "50%",
      height: 300,
    });
...
export default connect(mapStateToProps, { protectedTest })(
  withRouter(withCookies(TokboxPage))
);
