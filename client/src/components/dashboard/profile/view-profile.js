...
class ViewExpert extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(`${API_URL}/getExpertDetail/${currentUser.slug}`).then((res) => {
  ...
  componentWillMount() {
    ...
    axios
      .get(`${API_URL}/getExpertDetail/${this.state.currentUser.slug}`)
      .then((res) => {
        const expert = res.data[0];
  ...
  render() {
    return (
      <div>{this.state.loading ? this.renderPosts() : this.renderPosts()}</div>
    );
  }
}
