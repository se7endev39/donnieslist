  ...
  render() {
    const userList = (
      <div>
        <table>
          <tr>
            <th style={{ width: 20 + "%", paddingLeft: 10 + "px" }}>
              First Name
            </th>
          ...
          {this.state.users.map((user, i) => (
            <TableRow index={i} data={user} s={this.state} key={`TABLE_ROW_${i}`} />
          ))}
        </table>
      </div>
    );
...
export default connect(mapStateToProps, { protectedTest, GetActiveSessions })(
  withCookies(AdminSessionList)
);
