...
*/
      if (this.props.expertEmail === this.props.messageReceiverEmail) {
        console.log(
          "~~~~~ case 1: " +
            this.props.sessionOwnerUsername +
            " ---- " +
            this.props.messageSenderEmail
        );
        ...
        console.log(
          "~~~~~ case 2: " +
            this.props.sessionOwnerUsername +
            " ---- " +
            this.props.expertEmail
        );

        this.props
          .FetchExpertConversation(
            this.props.sessionOwnerUsername,
            this.props.messageReceiverEmail
          )
          .then((res) => {
            window.$(".Chating_msg_Here").animate({
              //                    scrollTo: window.$('.Chating_msg_Here ul').scrollHeight
  ...
  UNSAFE_componentWillMount() {
    ...
    window.$(document).ready(function () {
      window.$("#emojionearea1").emojioneArea({
        // pickerPosition: "left",
        // tonesStyle: "bullet",
        events: {
          change(editor, event) {
          ...
          keypress: function (editor, event) {
            ...
            if (event.keyCode === 13) {
              ...
              for (var x = 0; x < el.childNodes.length; x++) {
                if (
                  el.childNodes[x].tagName === undefined &&
                  el.childNodes[x].tagName !== "DIV"
                ) {
                  if (fullString === "") {
                ...
                if (el.childNodes[x].tagName === "IMG") {
                  fullString =
                    fullString + " " + el.childNodes[x].getAttribute("alt");
                }
              }
              ...
              if (j !== "") {
                ...
                self.props.ExpertSendReply(
                  self.props.sessionOwnerUsername,
                  self.props.messageReceiverEmail,
                  self.props.messageSenderEmail,
                  composedMessage,
                  date
                );
                editor[0].innerHTML = "";
                window.$(".emojionearea-editor")[0].innerHTML = "";
                window.$(".emojionearea-editor").html("");
      ...
      <form onSubmit={handleSubmit(this.handleFormSubmit.bind(this))}>
        ...
        <div className="form-group">
          <textarea
            name="emojionearea1"
            id="emojionearea1"
            value={this.state.emojionearea1}
